return {
  ...
  dependencies = {
...

    {
      ...
      cond = function()
        if vim.fn.has 'nvim-0.10' and require('utils').is_darwin() then
          return vim.fs.root(0, 'buildServer.json')
        else
          return false
        end
      end,
  ...
  config = function()
    ...
    if utils.is_darwin() and utils.is_xcode_project() then
      local xcodebuild = require 'xcodebuild.integrations.dap'

      local codelldbPath = os.getenv 'HOME' .. '/code/codelldb-aarch64-darwin/extension/adapter/codelldb'
